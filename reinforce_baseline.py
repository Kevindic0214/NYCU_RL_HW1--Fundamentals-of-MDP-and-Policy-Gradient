...
class REINFORCE:
    ...
    def update(self):
        ...
        # 計算值網路預測
        states = [torch.FloatTensor([s]) for s in self.states]  # 假設儲存了狀態序列
        values = torch.cat([self.value_net(s) for s in states]).squeeze()
        ...
        entropy_loss = -torch.stack(self.entropies).mean()  # 最大化熵
