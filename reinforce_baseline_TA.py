...
def train(lr=0.01):
    ...
    for i_episode in count(1):
        ...
        state, _ = env.reset()  # 更新reset方法返回值
        ep_reward = 0
        t = 0
        ...
        while True:
            ...
            state, reward, terminated, truncated, _ = env.step(action)  # 更新step方法返回值
            done = terminated or truncated  # 使用terminated和truncated來確定done狀態
            ep_reward += reward
        ...
        writer.add_scalar('Learning Rate', scheduler.get_last_lr()[0], i_episode)  # 更新get_lr()為get_last_lr()[0]
        ########## END OF YOUR CODE ##########
...
if __name__ == '__main__':
    ...
    random_seed = 10  
    lr = 0.002
    env = gym.make("LunarLander-v2", render_mode="human")  # 添加render_mode參數
    env.reset(seed=random_seed)  # 使用新的seed設定方式
